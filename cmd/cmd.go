...
import (
	...
	"github.com/issuerail/server/config"
)

var (
	app        = kingpin.New("server", "issuerail server").Version(config.Version + "-build-" + config.Build)
	configFile = app.Flag("config-file", "Config file (.yml)").Required().String()
	_          = app.Flag("listen-url", "Listen URL (host:port)").Required().String()
)
